...
class LeafMotionTests(unittest.TestCase):
    def test_every_leaf_variant_is_bound_without_overriding_other_animations(self):
        ...
        for definition in definitions:
            ...
            self.assertEqual(definition['frames'], [0, 1, 2, 3])
            self.assertEqual(definition['ticks_per_frame'] * len(definition['frames']), 80)
            self.assertTrue(definition['blend_frames'])

    def test_all_frames_preserve_pixels_and_alpha_with_at_most_one_pixel_of_motion(self):
        ...
        for name in sources:
            with self.subTest(texture=name):
                ...
                self.assertEqual((w, h), (width, height*4))
                size = width * height * 4
                self.assertEqual(frames[:size], original)
                self.assertEqual(frames[2*size:3*size], original)
